...
export function StripeFeeCalculator() {
  ...
  const [feePercentage, setFeePercentage] = useState<number | ''>(2.9);
  ...
  return (
    <Card className="w-full bg-white shadow-[8px_8px_0_0_rgba(0,0,0,1)] border-[3px] border-black rounded-[24px]">
      <CardHeader className="bg-[#f3e5ca] border-b-[3px] border-black rounded-t-[21px]">
        <CardTitle className="text-2xl font-black">Stripe Fee Calculator</CardTitle>
        <CardDescription className="text-black font-bold">Find out Stripe's exact cut for domestic payments.</CardDescription>
      </CardHeader>
        ...
        {result && (
          <div className="mt-6 space-y-4">
            <div className="bg-[#4a8eff] p-6 rounded-2xl border-[3px] border-black text-center shadow-[4px_4px_0_0_#000]">
              ...
              <p className="text-sm text-indigo-700 mt-2 font-bold">Total Fees: ${result.totalFee.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
            </div>
            ...
            <div className="bg-green-50 p-6 rounded-xl border-[3px] border-black text-center">
              ...
              <p className="text-xs text-green-700 mt-1">You need to charge the customer this amount.</p>
            </div>
          </div>
        )}
      </CardContent>
