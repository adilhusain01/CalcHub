...
export default async function EmbedCalculatorPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const calc = calculatorsList.find(c => c.slug === slug);
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="mt-3 text-center text-xs text-gray-400 font-medium">
        Powered by <a href="/" target="_blank" className="hover:text-gray-600 transition-colors">CalcTree</a>
      </div>
    </div>
  );
}
